...
def getPacketLevelInfo(flow, label):
    ...
    out_file = open(sys.argv[1] + '_output_' + TIMEOUT + '_flows','a')
    #out_file_udp = open('_output_flows_udp','a')
    #out_file_tcp = open('_output_flows_tcp','a')
    out_file_udp = open(sys.argv[1] + '_output_' + TIMEOUT + '_flows_udp','a')
    out_file_tcp = open(sys.argv[1] + '_output_' + TIMEOUT + '_flows_tcp','a')
    ...
    if proto == 'TCP':
        out_file_tcp.write(final_out + '\n') 
    else:
        out_file_udp.write(final_out + '\n')
